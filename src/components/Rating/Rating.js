...
import { checkRatingsList } from "../../services/authService";
...
function Rating() {
    ...
    useEffect(() => {
        const scanRatingsList = async () => {
            try{
                const mediaID = await checkRatingsList(); 
                const list = []
                for (let media of mediaID.ratings){
    ...
    if (mediaList.length === 0) {
        return <div className="display-msg">No user ratings found!</div>;
    }
